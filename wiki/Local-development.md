...
Stops prior validator, fetches `so/klend.so` if missing, `anchor build`.
...
`scripts/seed_localnet.ts` funds the fixture wallets, mints the dummy tokens, then
calls `cli init` — the same bootstrap devnet and mainnet use. Idempotent:

| Step | Instruction | Skip if |
|------|-------------|---------|
| — | fund admin + payer, `bootstrap_dummy_mints` (CCC / TTT, 100M each to admin) | never — always tops up |
...
`cli sync-env` (called from seed) merges backend keys and writes frontend `.env.local` files from `deployments/localnet.json`. Manual copy-paste is no longer required.

Re-run sync alone (after editing `deployments/localnet.json`):

```bash
cd wrap-stablecoin && pnpm cli sync-env --network localnet
```
...
| `pnpm cli sync-env --network localnet` | Re-apply `deployments/localnet.json` → env files |
...
- **Program ID** is `declare_id!`, read from the tracked `idl/wrap_stablecoin.json` by both the validator launcher and `cli/network.ts`, and printed by `anchor run local`. Never taken from `target/deploy/wrap_stablecoin-keypair.json`: `anchor build` regenerates that file whenever it is absent, so trusting it would load the validator's program at an address the client never uses. Backend `PROGRAM_ID` is written by env sync.

...
# declare_id!, via the committed IDL — the same source cli/network.ts uses. Never
# the deploy keypair: that file is regenerated by `anchor build` whenever it is
# absent, so trusting it would load the validator's program at an address the
# client and every PDA do not use.
local_env_resolve_program_id() {
  local idl="$ROOT/idl/wrap_stablecoin.json"
  if [[ ! -f "$idl" ]]; then
    echo "missing IDL: $idl (regenerate it with anchor idl build)" >&2
    return 1
  fi
  PROGRAM_ID="$(node -p "require('$idl').address ?? ''")"
  if [[ -z "$PROGRAM_ID" ]]; then
    echo "no address in $idl" >&2
    return 1
  fi
}
